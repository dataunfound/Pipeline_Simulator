...
## Key Features
* **Smart Instruction Parsing:** Converts raw text into structured dictionaries. It is fully case insensitive (`add` and `ADD` work the same) to provide a better user experience.
...
* **Smart Hazard Detection:** Actively scans the active stages to find data dependencies. When a hazard is found, it injects a STALL bubble and dynamically reports exactly which register caused the delay (`STALL (Wait R1)`).
